...
describe("buildPaneCommand", () => {
  it("builds a plain cwd command when the pane has no command", () => {
    expect(
      buildPaneCommand({ title: "shell", command: "" }, "/tmp/my repo")
    ).toBe("cd '/tmp/my repo'");
  });
  ...
  it("falls back to command when the pane does not accept override", () => {
    expect(
      buildPaneCommand({ title: "editor", command: "nvim" }, "/tmp/my repo", {
...
describe("interpolatePlaceholders", () => {
  it("leaves {branch} raw (no shell quoting)", () => {
    expect(
      interpolatePlaceholders("echo {branch}", { branch: "feat/test" })
    ).toBe("echo feat/test");
  });

  it("preserves whitespace around placeholders (matches existing {branch} behavior)", () => {
    expect(
      interpolatePlaceholders("echo   {branch}   done", { branch: "feat/test" })
    ).toBe("echo   feat/test   done");
  });
  ...
  it("interpolates multiple placeholders of different kinds", () => {
    expect(
      interpolatePlaceholders("echo {branch} {missing}", { branch: "feat/x" })
    ).toBe("echo feat/x ");
  });
});
